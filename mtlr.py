...
alldata_trn = pd.merge(left=iddata_trn.drop(columns="event"), right=alldata, left_on='id', right_on='id').drop(
    columns=["id"]).astype({'event': int})
alldata_tst = pd.merge(left=iddata_tst.drop(columns="event"), right=alldata, left_on='id', right_on='id').drop(
    columns=["id"]).astype({'event': int})
alldata_val = pd.merge(left=iddata_val.drop(columns="event"), right=alldata, left_on='id', right_on='id').drop(
    columns=["id"]).astype({'event': int})
...
alldata_tst = pd.merge(left=iddata_tst.drop(columns="event"), right=alldata_tmp.drop(columns="maxtime"), left_on='id',
                       right_on='id').drop(columns=["id"]).astype({'event': int})
...
json_loc = here / f'{CLUSTERID}-{PROCID}-mtlr.cbor'
